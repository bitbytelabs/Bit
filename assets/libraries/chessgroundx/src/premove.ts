...
const pawn = (color: cg.Color): Mobility => (x1, y1, x2, y2) => diff(x1, x2) < 2 && (color === 'white' ?
...
const king = (color: cg.Color, rookFiles: number[], canCastle: boolean): Mobility => (x1, y1, x2, y2) => diff(x1, x2) < 2 && diff(y1, y2) < 2 || canCastle && y1 === y2 && y1 === (color === 'white' ? 0 : 7) && (x1 === 4 && (x2 === 2 && rookFiles.includes(0) || x2 === 6 && rookFiles.includes(7)) || rookFiles.includes(x2));
...
function memoizeDistance(): (dist: number) => Mobility {
  ...
  return (dist: number) => {
    const key = `${dist}`;
    if (!(key in cache)) if (isSafeKey(key)) {
...
function builtinMobility(variant: string, chess960: boolean, bd: cg.BoardDimensions): (boardState: cg.BoardState, key: cg.Key, canCastle: boolean) => Mobility {
  switch (variant) {
    case 'xiangqi':
    case 'manchu':
