...
class UniversalBoardDrawer {
  ...
  createArrowBetweenPositions(from, to, config) {
    ...
    const arrowElem = typeof existingArrowElem === 'object' ? existingArrowElem : this.document.createElementNS('http://www.w3.org/2000/svg', 'polygon');
    ...
    const arrowPoints = [{
      ...
      x: fromX - lineWidth / 2,
      y: fromY - distance + arrowheadHeight
    }, {
      x: fromX - arrowheadWidth / 2,
      y: fromY - distance + arrowheadHeight
    }, {
      x: fromX,
      y: fromY - distance
  ...
  removeAllDebugDots() {
    this.addedShapes.filter(shapeObj => shapeObj.type == 'debugDot').forEach(debugDotObj => {
  ...
  updateShapes() {
    ...
    this.addedShapes.filter(shapeObj => shapeObj.type != 'debugDot').forEach(shapeObj => {
    ...
    for (let y = 0; this.boardDimensions.height > y; y++) {
      for (let x = 0; this.boardDimensions.width > x; x++) {
        this.squareSvgCoordinates.push({
          coordinates: [x + 1, y + 1],
          positions: [this.squareWidth / 2 + this.squareWidth * x, this.squareHeight / 2 + this.squareHeight * y]
        });
      }
    }
  }
  transferAttributes(fromElem, toElem) {
    if (fromElem && fromElem?.attributes && toElem) {
      [...fromElem.attributes].forEach(attr => toElem.setAttribute(attr.name, attr.value));
    }
  }
  updateDimensions() {
    ...
    if (this.adjustSizeByDimensions) {
      if (this.boardDimensions.width > this.boardDimensions.height) {
        ...
        if (boardHeight !== newHeight) {
          if (!this.adjustSizeConfig?.noTopAdjustment) boardPositionTop += (boardHeight - newHeight) / 2;
        ...
        if (boardWidth !== newWidth) {
          if (!this.adjustSizeConfig?.noLeftAdjustment) boardPositionLeft += (boardWidth - newWidth) / 2;
          boardWidth = newWidth;
        }
      }
    }
  ...
  getCoordinatesFromInputPosition(e) {
    ...
    return isOutOfBounds ? [null, null] : [Math.floor(relativeX / this.squareWidth) + 1, Math.floor(relativeY / this.squareHeight) + 1];
  }
  handleMouseEvent(e) {
  ...
  addSquareListener(square, cb) {
    this.customActivityListeners.push({
      square,
      cb
    });
    return {
  ...
  async demo() {
    ...
    const createText = (square, opacity, size, x, y) => {
      return [BoardDrawer.createShape('text', square, {
        size,
        text: '♥️',
    ...
    for (let sum = 0; sum <= totalSteps; sum++) {
      ...
      for (let x = 0; x <= sum; x++) {
        ...
        createdElements.push(BoardDrawer.createShape('rectangle', square, {
          style: rectStyle
        }));
        if (x > 0 && y > 0) {
    ...
    await delay(1000);
    createdElements = createdElements.filter(x => x).reverse();
