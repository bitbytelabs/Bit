...
function geoGebraDotCommands(data) {
  ...
  const wCmd = `wPoints = Sequence((i, Element(${wList}, i)), i, 1, ${w.length})`;
  const bCmd = `bPoints = Sequence((i, Element(${bList}, i)), i, 1, ${b.length})`;
  return {
    wCmd,
    bCmd
  };
...
function getUniqueMoves(moves) {
  ...
  for (let i = 0; i < moves.length; i++) {
    const m = moves[i];
    const key = m.player[0] + ',' + m.player[1] + '|' + m.opponent[0] + ',' + m.opponent[1] + '|' + m.profile;
    if (!seen.has(key)) {
      seen.add(key);
      if (isSafeKey(write++)) {
        cleaned[write++] = m;
...
function highlightSetting(targetElem, cb) {
  ...
  const subtleElems = ['#settings-header', '#settings-panels', '#setting-container'].map(x => document.querySelector(x)).filter(x => x);
...
function getDepthFromElo(elo) {
  return elo >= 3100 ? 18 : elo >= 3000 ? 16 : elo >= 2900 ? 14 : elo >= 2800 ? 12 : elo >= 2700 ? 10 : elo >= 2600 ? 8 : elo >= 2400 ? 7 : elo >= 2200 ? 6 : elo >= 2000 ? 5 : elo >= 1800 ? 4 : elo >= 1600 ? 3 : elo >= 1400 ? 2 : elo >= 1200 ? 1 : 1;
}
function allowOnlyNumbers(e) {
  return e.charCode >= 48 && e.charCode <= 57;
}
function getBasicFenLowerCased(fenStr) {
  return fenStr?.replace(/\[.*?\]/g, '') // remove [] (and anything between) 
  ?.split(' ')?.[0]?.toLowerCase();
}
...
async function getProfileNames() {
  ...
  console.error('Could not find profile names!', {
    ...settingFilterObj,
    gmConfigKey,
    config
  });
...
function eloToTitle(elo) {
  return elo >= 2900 ? "Cheater" : elo >= 2500 ? "Grandmaster" : elo >= 2400 ? "International Master" : elo >= 2300 ? "Fide Master" : elo >= 2200 ? "National Master" : elo >= 2000 ? "Expert" : elo >= 1800 ? "Tournament Player" : elo >= 1600 ? "Experienced" : elo >= 1400 ? "Intermediate" : elo >= 1200 ? "Average" : elo >= 1000 ? "Casual" : "Beginner";
}
...
const engineEloArr = [{
  ...
  elo: 2700,
  data: 'go movetime 1500'
}, {
  elo: 2800,
  data: 'go movetime 3000'
}, {
  elo: 2900,
  data: 'go movetime 5000'
}, {
  elo: 3000,
  data: 'go movetime 10000'
}];
...
function parseUCIResponse(response) {
  const keywords = ['id', 'name', 'author', 'uciok', 'readyok', 'bestmove', 'option', 'info', 'score', 'pv', 'mate', 'cp', 'wdl', 'depth', 'seldepth', 'nodes', 'time', 'nps', 'tbhits', 'currmove', 'currmovenumber', 'hashfull', 'multipv', 'refutation', 'line', 'stop', 'ponderhit', 'ucs', 'position', 'startpos', 'moves', 'files', 'ranks', 'pocket', 'template', 'variant', 'ponder', 'Fen:', 'bmc'];
  const data = {};
...
function isBelowVersion(versionStr, targetVersion) {
  return versionStr.localeCompare(targetVersion, undefined, {
    numeric: true
  }) === -1;
}
function formatProfileName(profileNameStr) {
  return profileNameStr.trim().replace(/[^a-zA-Z0-9]/g, '').replace(/\s+/g, '');
}
function initNestedObject(obj, keys) {
  keys.reduce((acc, key) => {
    if (!acc[key]) if (isSafeKey(key)) {
      acc[key] = {};
    }
    return acc[key];
  }, obj);
...
async function isEngineIncompatible(engineName, profileName, skipSabCheck) {
  ...
  async function check(pN) {
    const enginesRequiringSAB = [
    // Requiring SharedArrayBuffer
    'stockfish-17-wasm', 'stockfish-16-1-wasm', 'stockfish-14-nnue', 'fairy-stockfish-nnue-wasm', 'lc0'];
