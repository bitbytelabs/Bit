...
class BackendInstance {
  constructor(domain, instanceID, chessVariant, onLoadCallbackFunction) {
    ...
    this.profileVariables = class {
      ...
      static async create(t, profile) {
        ...
        instance.chessVariant = isVariant960(chessVariant) ? formatVariant('chess') : formatVariant(chessVariant || variantFromConfig || 'chess');
    ...
    this.CommLinkReceiver = this.CommLink.registerListener(`frontend_${this.instanceID}`, packet => {
      try {
        if (this.instanceReady) return this.processPacket(packet);
  ...
  async enforceMoveSuggestions(profile) {
    ...
    const noEnforcementNeeded = moveSuggestionAmount <= 0 || isEngineCalculating || activeMoveMarkings?.length > 0;
    console.log({
  ...
  getArrowStyle(type, fill, opacity) {
    const getBaseStyleModification = (f, o) => ['stroke: rgb(0 0 0 / 50%);', 'stroke-width: 2px;', 'stroke-linejoin: round;', `fill: ${fill || f};`, `opacity: ${opacity || o};`].join('\n');
    switch (type) {
      case 'best':
  ...
  Interface = {
    boardUtils: {
      ...
      updateBoardFen: async fen => {
        if (this.currentFen !== fen) {
          ...
          if (this.debugLogsEnabled) {
            ...
            console.warn('[Logical Change Detection] New board FEN received:', `${origin}/Bit/board/?fens=${fensString}&o=${this.lastOrientation}`, {
              fen,
              moveObj
            });
          }
          ...
          if (movedPieceLowered === 'k') {
            const kingColor = moveObj?.movedPiece === movedPieceLowered ? 'b' : 'w';
            if (!this.kingMoved.includes(kingColor)) this.kingMoved += kingColor;
          }
      ...
      updateBoardOrientation: orientation => {
        if (orientation != this.lastOrientation) {
          ...
          if (orientation == 'b') evalBarElem.classList.add('reversed');else evalBarElem.classList.remove('reversed');
    ...
    updateEval: async (centipawnEval, mate, profile) => {
      ...
      if (mate) advantage = centipawnEval > 0 ? 1 : 0;
      updatePipData({
        'eval': advantage,
        playerColor,
        centipawnEval
  ...
  async renderMetric(fen, profile) {
    ...
    if (!(renderSquarePlayer || renderSquareEnemy || renderSquareContested || renderSquareSafe || renderPieceEnemyCapture)) return;
    const playerColor = await this.getPlayerColor(profile);
    const addedMetrics = [];
    ...
    if (renderPiecePlayerCapture) analResult.player.forEach(piece => renderDanger(piece, '💧'));
    if (renderPieceEnemyCapture) analResult.enemy.forEach(piece => renderDanger(piece, '🩸'));
    if (renderSquarePlayer) analResult.squares.playerOnly.forEach(pos => renderPlayerOnly(pos));
    if (renderSquareEnemy) analResult.squares.enemyOnly.forEach(pos => renderEnemyOnly(pos));
    if (renderSquareContested) analResult.squares.contested.forEach(obj => renderContested(obj));
    if (renderSquareSafe) analResult.squares.safe.forEach(pos => renderSafe(pos));
    ...
    if (renderOnExternalSite) {
      ...
      const metricsWithoutElem = addedMetrics.map(x => ({
        ...x,
        elem: null
      }));
  ...
  async setEngineElo(elo, didUserUpdateSetting, didUpdateAdvancedElo, profile) {
    ...
    if (enableAdvancedElo) {
      ...
      if (!limitStrength) this.setEngineLimitStrength(false, profile);
      if (advancedEloMaxError > 0) this.setEngineMaxError(advancedEloMaxError, profile);
      if (advancedEloProbability > 0) this.setEngineProbability(advancedEloProbability, profile);
      ...
      if (limitStrength) {
        ...
        if (elo !== 3200) {
          const depth = getDepthFromElo(elo);
          this.pV[profile].searchDepth = depth;
          if (didUserUpdateSetting && !didUpdateAdvancedElo) toast.message(`${engineNotLimitedSkillLevel} | ${searchDepthMsg} ${depth}`, 8000);
        } else {
          this.pV[profile].searchDepth = null;
          updatePipData({
            'goalDepth': null
          });
          if (didUserUpdateSetting) toast.message(engineNoLimitations, 8000);
        }
      }
    }
  }
  ...
  setChessFont(chessFontStr) {
    ...
    const chessboardElems = [this.instanceElem.querySelector('.chessboard-components'), this.instanceElem.querySelector('.pseudoground-x')?.parentElement];
  ...
  async engineStartNewGame(variant, profile) {
    ...
    if (engineName !== 'lc0') this.sendMsgToEngine(`setoption name UCI_AnalyseMode value true`, profile); // required for threads, etc.
  ...
  async updateSettings(updateObj) {
    ...
    const advancedEloKeys = [this.configKeys.enableAdvancedElo, this.configKeys.advancedEloDepth, this.configKeys.advancedEloSkill, this.configKeys.advancedEloMaxError, this.configKeys.advancedEloProbability, this.configKeys.advancedEloHash, this.configKeys.advancedEloThreads];
    const didUpdateVariant = findSettingKeyFromData(this.configKeys.chessVariant);
    const didUpdateElo = [this.configKeys.engineElo, ...advancedEloKeys].find(key => findSettingKeyFromData(key));
    const didUpdateAdvancedElo = advancedEloKeys.find(key => findSettingKeyFromData(key));
    ...
    if (didUpdateVariant || didUpdate960Mode) {
      ...
      if (didUpdateChessFont) this.setChessFont(await this.getConfigValue(this.configKeys.chessFont));
      if (didUpdateChessEngine || didUpdateEngineEnabled && isEngineEnabled) {
        if (didUpdateChessEngine) console.log('Kill and load engine', profile, 'since the engine type was changed');else if (didUpdateEngineEnabled) console.log('Kill and load engine', profile, 'since the engine was enabled');
      ...
      if (didUpdateElo) this.setEngineElo(await this.getConfigValue(this.configKeys.engineElo, profile), true, didUpdateAdvancedElo, profile);
      if (didUpdateNodes) this.setEngineNodes(await this.getConfigValue(this.configKeys.engineNodes, profile), profile);
      if (didUpdateLc0Weight) this.setEngineWeight(await this.getConfigValue(this.configKeys.lc0Weight, profile), profile);
      if (didUpdateMultiPV) this.setEngineMultiPV(await this.getConfigValue(this.configKeys.moveSuggestionAmount, profile), profile);
      if (didUpdate960Mode) this.set960Mode(useChess960, profile);
    }
  }
  isPawnOnPromotionSquare(currentFen) {
    ...
    if (pawnOnPromotionSquare) return true;
    return false;
  }
  ...
  isCorrectAmountOfBoardChanges(lastFen, newFen) {
    ...
    */
    ...
    if (diff > 5) this.engineStartNewGame();
  ...
  isFenChangeLogical(lastFen, newFen) {
    ...
    if (this.debugLogsEnabled) console.warn('[Logical Change Detection] Is FEN change logical:', {
      correctAmountOfChanges,
      isAbnormalPieceChange
    });
  ...
  async displayFeedback(currentFen) {
    ...
    const display = async (from, to, cp, category, label, profileName) => {
      ...
      if (feedbackOnExternalSite) {
        // Create a copy without modifying the original array
        const feedbacksWithoutElem = addedFeedbacks.map(x => ({
          ...x,
          elem: null
        }));
    ...
    for (const profileObj of profiles.filter(p => p.config.enableMoveRatings || p.config.enableEnemyFeedback)) {
      ...
      if (isChangeLogical && lastFen && currentFen) {
        ...
        this.MoveEval.eval([from, to], {
          'fen': fromFen,
          'depth': feedbackEngineDepth
        }, resultObj => {
  ...
  async calculateBestMoves(currentFen, skipValidityChecks, specificMovesObj) {
    const profiles = await getProfiles();
    const shouldCalculate = p => p.config.engineEnabled && (!p.config.movesOnDemand && !specificMovesObj || specificMovesObj && p.config.movesOnDemand);
    ...
    profiles.filter(p => shouldCalculate(p)).forEach(async profile => {
      ...
      this.pV[profileName].pendingCalculations.push({
        'fen': currentFen,
        'startedAt': Date.now(),
        'finished': false
      });
      ...
      if (specificMovesObj?.moves) specificMoves = ' searchmoves ' + specificMovesObj.moves.join(' ');
  ...
  async displayMoves(moveObjects, profile) {
    ...
    moveObjects.forEach(moveObj => {
      const spokenText = moveObj.player?.map(x => x.split('').map(x => `"${x}"`).join(' ') // e.g a1 -> "a" "1"
      ).join(', ');
  ...
  async engineMessageHandler(msg, profile) {
    ...
    if (msg.includes('info')) {
      if (data?.multipv === '1' || ['lozza-5', 'lc0'].includes(await this.getEngineType(profile))) {
        ...
        if (data?.cp) this.Interface.updateEval(data.cp, false, profile);
        if (data?.mate) this.Interface.updateEval(data.mate, true, profile);
      }
    }
    if (data?.wdl) {
    ...
    if (data?.pv && isMessageForCurrentFen) {
      ...
      if (moves?.length === 1)
        // if no opponent move guesses yet
      ...
      if (markingLimit !== 0 && topMoveObjects.length === markingLimit - removedDuplicateMoveAmount && (!onlyShowTopMoves || isSearchInfinite && !isMovetimeLimited) // handle infinite search, cannot only show top moves when search is infinite
    ...
    if (data?.bestmove) {
      if (oldestUnfinishedCalcRequestObj) oldestUnfinishedCalcRequestObj.finished = true;
      ...
      if (isMessageForCurrentFen && this.pV[profile].activeGuiMoveMarkings.length === 0) {
        ...
        if (topMoveObjects?.length === 0) {
          topMoveObjects = [];
          topMoveObjects.push({
            'player': [data.bestmove.slice(0, 2), data.bestmove.slice(2, data.bestmove.length)],
            'opponent': [null, null],
            'ranking': 1
          });
        } else {
        ...
        if ((await this.getEngineType(profile)) === 'lc0') {
          updatePipData({
            'nodes': this.pV[profile].engineNodes,
            'goalDepth': null
          });
        } else {
          updatePipData({
            'depth': this.pV[profile].searchDepth,
            'goalNodes': null
          });
        }
        if (isDelayActive) {
          ...
          if (markingLimit !== 0) this.displayMoves(topMoveObjects, profile);
        }
      }
    }
    ...
    if (msg === 'uciok' && ((await this.getEngineType(profile)) === 'lc0' || (await this.getEngineType(profile)) === 'lozza-5')) {
  ...
  async loadEngine(profileName, engineName, attempt = 0) {
    ...
    function restartEngine(name, e) {
      ...
      if (!errorMessage.includes('memory access')) {
        if (!errorMessage.includes('[object ErrorEvent]')) toast.error(`Engine "${name}" crashed due to "${errorMessage}"!`, 5e3);
        return;
      }
      const crashKey = `${profileName}:${name}`;
      const memoryCrashCount = (this.engineCrashCounts[crashKey] || 0) + 1;
      if (isSafeKey(crashKey)) {
    ...
    function loadLilaStockfish(workerName) {
      const stockfish = new Worker(`../app/assets/engines/lila-stockfish/${workerName}.js`, {
        type: 'module'
      });
      let stockfish_loaded = false;
      stockfish.onmessage = async e => {
        if (e.data === true) {
          ...
          this.engines.push({
            'type': profileChessEngine,
            'engine': (method, a) => stockfish.postMessage({
              method: method,
              args: [...a]
            }),
            'sendMsg': msg => stockfish.postMessage({
              method: 'uci',
              args: [msg]
            }),
            'worker': stockfish,
          ...
          startGame.bind(this)(workerName === 'f14-worker' ? formatVariant(this.pV[profileName].chessVariant) : 'chess');
        } else if (e.data) {
          processEngineMessage(e.data);
        }
      };
      const waitStockfish = setInterval(() => {
        ...
        stockfish.postMessage({
          method: 'acas_check_loaded'
        });
      }, 100);
    }
    function loadLc0() {
      const lc0 = new Worker('../app/assets/engines/zerofish/zerofishWorker.js', {
        type: 'module'
      });
      ...
      lc0.onmessage = async e => {
        if (e.data === true) {
          ...
          this.engines.push({
            'type': profileChessEngine,
            'engine': (method, a) => lc0.postMessage({
              method: method,
              args: [...a]
            }),
            'sendMsg': msg => lc0.postMessage({
              method: 'zero',
              args: [msg]
            }),
            'worker': lc0,
    ...
    function loadMaia2() {
      const maia = new Worker('../app/assets/engines/Maia2/worker.js', {
        type: 'module'
      });
      ...
      maia.onmessage = async e => {
        if (e.data === true) {
          ...
          this.engines.push({
            'type': profileChessEngine,
            'engine': (method, a) => maia.postMessage({
              method: method,
              args: [...a]
            }),
            'sendMsg': msg => maia.postMessage({
              method: 'uci',
              args: [msg]
            }),
            'worker': maia,
  ...
  async setupEnvironment(startpos, dimensions) {
    if (this.environmentSetupRun) return;
    ...
    try {
      ...
      for (const profileName of Object.keys(this.pV)) {
        ...
        if (profileVariant !== 'chess') variant = profileVariant;
        const onlyCalculateOwnTurn = await this.getConfigValue(this.configKeys.onlyCalculateOwnTurn, profileName);
      ...
      "chess", "crazyhouse", "chess960", "kingofthehill", "threecheck", "3check", "antichess", "atomic", "horde", "racingkings", "bughouse", "doublechess", "loserschess", "capablanca", "shatranj", "kingscourt", "reversechess", "crazychess", "normal", "fischerandom", "losers", "shako", "knightmate", "perfect"].includes(formattedChessVariant);
      if (shouldSwitchFont) chessFont = 'merida';
      ...
      if (!earlyBestMoveRequest) this.calculateBestMoves(fen);
