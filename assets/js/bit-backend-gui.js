...
if (window?.SharedArrayBuffer) {
  ...
  ['advancedEloThreads', 'advancedEloHash'].forEach(x => {
...
(async () => {
  const GM_info = typeof USERSCRIPT?.getInfo === 'function' ? await USERSCRIPT?.getInfo() : USERSCRIPT?.GM_info;
  if (userscriptInfoElem && typeof USERSCRIPT === 'object' && GM_info) {
    const platform = GM_info?.platform || GM_info?.userAgentData || {
      'platform': 'Unknown'
    };
...
function displayNoUserscriptNotification(isEnable) {
  if (isEnable) installNotificationElem.classList.add('hidden');else installNotificationElem.classList.remove('hidden');
}
function displayTOS() {
...
function fillChessVariantDropdowns(arr) {
  ...
  chessVariantDropdownElems.filter(elem => !elem.getAttribute('filled-successfully')).forEach(elem => {
...
function setInputValue(elem, val) {
  ...
  if (isCheckbox) {
    ...
    if (isDropdown) {
      // highlight dropdown selected item
      ...
      dropdownElems.forEach(elem => {
        if (elem?.dataset?.value === val) elem.classList.add(selectedItemKey);else elem.classList.remove(selectedItemKey);
      });
    }
...
async function makeSettingChanges(inputElem) {
  ...
  if (alreadyProcessed && inputElem.dataset.key === 'pip') return;
  switch (inputElem.dataset.key) {
    case 'themeColorHex':
      if (isSafeKey('background-color')) {
    ...
    case 'chessEngine':
      ...
      if (value === 'maia2') {
        ...
        advancedEloInputs.forEach(input => input.setAttribute('disabled', 'true'));
        normalEloInput.removeAttribute('disabled');
      } else {
        advancedEloEnableInput.removeAttribute('disabled');
        advancedEloInputs.forEach(input => input.removeAttribute('disabled'));
      }
      break;
    ...
    case 'pip':
      ...
      if (value) startPictureInPicture();else if (document.pictureInPictureElement) await document.exitPictureInPicture();
      break;
    case 'pipBoard':
    ...
    case 'enableAdvancedElo':
      if (chessEngineInput.value === 'maia2') {
        if (advancedEloEnableInput.checked) advancedEloEnableInput.click();
        advancedEloEnableInput.setAttribute('disabled', 'true');
      }
      if (!normalEloInput) return;
      if (value) normalEloInput.setAttribute('disabled', 'true');else normalEloInput.removeAttribute('disabled');
      break;
  }
...
async function saveSetting(settingElem) {
  ...
  const settingObj = {
    'key': settingElem.dataset.key,
    'value': convertToCorrectType(elemValue)
  };
  const gmConfigKey = GLOBAL_VARIABLES.gmConfigKey;
  const config = await USERSCRIPT.getValue(gmConfigKey);
...
function toggleSelected(selectedElem, instanceID) {
  ...
  options.forEach(elem => {
    if (elem == selectedElem) elem.classList.add('selected');else elem.classList.remove('selected');
  });
  updateSettingsValues();
}
...
function initializeDropdown(dropdownElem) {
  ...
  function updateDropdown(showAll) {
    const listItems = [...listContainerElem.querySelectorAll('.dropdown-item')].filter(x => x?.dataset?.value);
    ...
    const filteredOptions = optionsArr.filter(option => words.every(word => {
    ...
    listItems.filter(elem => !elem.getAttribute('onclick-set')).forEach(elem => {
  ...
  updateDropdown(true);
  new MutationObserver(() => updateDropdown(true)).observe(listContainerElem, {
    childList: true,
    subtree: true
  });
}
function initGUI() {
  ...
  [...document.querySelectorAll('input[data-key]')].forEach(elem => {
    ...
    if (defaultVal) {
      ...
      if (!elem.placeholder) elem.placeholder = elem.dataset.defaultValue;
    }
    if (isRange) {
    ...
    elem.onchange = e => {
      ...
      if (e.target.value || e.target.checked) {
        saveSetting(elem);
        if (e?.target?.dataset?.key === 'displayMovesOnExternalSite' || e?.target?.dataset?.key === 'renderOnExternalSite' || e?.target?.dataset?.key === 'movesOnDemand') {
