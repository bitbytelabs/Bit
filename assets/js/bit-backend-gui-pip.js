...
async function renderPipBoards(from, to) {
  ...
  if (isNewSuggestion) {
    ...
    if (!cgElem) return;
    const canvas = await snapdom.toCanvas(cgElem, {
      fast: true
    });
    ...
    const canvas2 = await snapdom.toCanvas(container, {
      fast: true
    });
...
function updatePipContext() {
  ...
  const latestQueueArr = pipProcessingBestMove ? pipContextQueue.find(x => x?.[0]?.[1] === pipProcessingBestMove) : pipContextQueue?.[0];
  if (latestQueueArr) {
    ...
    for (const [cmd, args] of latestQueueArr) {
      if (cmd in ctx) {
        if (Array.isArray(args)) ctx[cmd](...args);else if (isSafeKey(cmd)) {
          ctx[cmd] = args;
        }
      }
    }
...
async function refreshPipView() {
  ...
  if (!eval && pipLastPipEval === null) eval = 0.5;else if (eval) pipLastPipEval = eval;
  ...
  if (pipData?.moveProgressText) {
    ...
    ctxQueue.push(['fillText', [pipData.moveProgressText, 12, pipHeaderHeight + headerOffset, pipMaxTextWidth]]);
  ...
  if (pipData.calculationTimeElapsed) {
    ...
    const timeFormatted = timeMs > 9999 ? `${(timeMs / 1000).toFixed(1)}s` : `${timeMs}ms`;
    const progressPercent = (progress * 100).toFixed(0);
    ctxQueue.push(['fillStyle', 'rgba(255, 255, 255, 0.5)']);
    ctxQueue.push(['font', `500 ${pipFontSizes.small}px Mona Sans`]);
    ctxQueue.push(['fillText', [`(${timeFormatted}, ${progressPercent}%)`, 120, 28]]);
  }
  ctxQueue.push(['fillStyle', 'white']);
  ...
  if (isBoard) {
    ...
    if (to === 'one)') {
      ...
      ctxQueue.push(['fillText', [`${from.toUpperCase()} ➔ ${to.toUpperCase()}`, 16, pipHeaderHeight + 100]]);
    }
    if (sFrom && sTo) {
      ctxQueue.push(['fillStyle', 'rgba(255, 255, 255, 0.5)']);
      ctxQueue.push(['font', `500 ${pipFontSizes.medium}px Mona Sans`]);
      ctxQueue.push(['fillText', [`2. (${sFrom.toUpperCase()} ➔ ${sTo.toUpperCase()})`, 16, pipHeaderHeight + 135]]);
    }
    if (tFrom && tTo) {
      ctxQueue.push(['fillText', [`3. (${tFrom.toUpperCase()} ➔ ${tTo.toUpperCase()})`, 180, pipHeaderHeight + 135]]);
    }
  }

  // PROGRESS BAR
  ctxQueue.push(['fillStyle', 'rgba(0, 0, 0, 0.1)']);
  ctxQueue.push(['fillRect', [0, pipHeaderHeight, (pipCanvas.width - pipEvalBarWidth) * progress, pipCanvas.height - pipHeaderHeight]]);
  ...
  if (playerColor === 'b') {
    ...
    ctxQueue.push(['fillRect', [pipCanvas.width - pipEvalBarWidth, 0, pipEvalBarWidth, pipCanvas.height * eval]]);
  } else {
    ctxQueue.push(['fillStyle', 'rgba(200, 200, 200, 1)']);
    ctxQueue.push(['fillRect', [pipCanvas.width - pipEvalBarWidth, 0, pipEvalBarWidth, pipCanvas.height]]);
    ctxQueue.push(['fillStyle', 'rgba(50, 50, 50, 1)']);
    ctxQueue.push(['fillRect', [pipCanvas.width - pipEvalBarWidth, 0, pipEvalBarWidth, pipCanvas.height * (1 - eval)]]);
  }

  // EVAL TEXT
  const centipawnEval = pipData?.centipawnEval / 100;
  if (!pipData?.mate && centipawnEval) {
    let yPosition = playerColor === 'w' ? centipawnEval < 0 ? 24 : pipCanvas.height - 12 : centipawnEval > 0 ? 24 : pipCanvas.height - 12;
    ...
    ctxQueue.push(['fillText', [evalText, headerWidth + [14, 11, 6, 2, 0, 0][evalText.length - 1], yPosition]]);
  }

  // STATUS BAR
  ctxQueue.push(['fillStyle', ['rgba(40, 40, 40, 0.9)', 'rgba(0, 255, 0, 1)', 'rgba(255, 0, 0, 1)'][pipData.isWinning ?? 0]]);
...
async function startPictureInPicture() {
  let width = 200,
    height = window.pipBoardActive ? 200 : 100;
  const pipWidth = width * 2,
    pipHeight = height * 2;
