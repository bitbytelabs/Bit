...
function swapColorsInRank(rank) {
  return rank.split('').map(c => /[A-Z]/.test(c) ? c.toLowerCase() : /[a-z]/.test(c) ? c.toUpperCase() : c).join('');
}
...
function boardToTensor(fen) {
  ...
  for (let r = 0; r < 8; r++) {
    ...
    for (const char of rows[r]) {
      ...
      if (isSafeKey(idx * 64 + (7 - r) * 8 + file)) {
  ...
  castlingFlags.forEach((f, i) => {
    if (f) tensor.fill(1, (13 + i) * 64, (14 + i) * 64);
  });
  if (enPassant !== '-') {
    const file = enPassant.charCodeAt(0) - 97;
    const rank = 7 - (parseInt(enPassant[1]) - 1);
    if (isSafeKey(17 * 64 + rank * 8 + file)) {
...
function preprocess(fen, eloSelf, eloOppo, ChessLib) {
  if (allPossibleMoves === null || allPossibleMovesReversed === null) throw new Error('Move data not loaded. Call loadMoves() first.');
  ...
  board.moves({
    ...
    if (idx !== undefined) if (isSafeKey(idx)) {
      legalMoves[idx] = 1;
    }
  });
  return {
...
function policyToUciLines(fen, policyObj, multipv = 1) {
  ...
  for (let i = 0; i < Math.min(multipv, sortedMoves.length); i++) {
    ...
    lines.push(`info depth 1 seldepth 1 multipv ${i + 1} score cp ${cp} nodes 1 nps 1 time 0 pv ${move}`);
  }
